  ...
  public LoginServiceResult sendEmailVerifyCode(LoginPO loginPO) {
    ...
    if (operatorVO == null) {
      ...
      if (StringUtils.isNotBlank(operatorId)) {
        OperatorVO currentOperator = operatorDao.getById(operatorId);
        if (currentOperator != null) {
          operatorVO = currentOperator;
          operatorVO.setEmail(email);
          if (log.isDebugEnabled()) {
            log.debug(
                "Using authenticated operator {} to send verification code to new email {}",
                operatorId,
                email);
          }
        }
    ...
    if (operatorVO == null) {
      // Return success even if email doesn't exist to prevent enumeration
      log.debug(
          "Email verification requested for non-existent email {} without authenticated user context",
          email);
      return new LoginServiceResult(ServiceResultType.SUCCESS);
    }
    ...
    context.put("username", StringUtils.defaultIfBlank(operatorVO.getUsername(), email));
    context.put("email", email);
